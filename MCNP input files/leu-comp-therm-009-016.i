...
c     - x-value is given by +/-(15*2.54 + Sc).
   51    0   -63 +62  -83 +82  -95 +93             $
             fill=11(-44.03 0.00 0.00)             $  left cluster
   52    0   -65 +64  -83 +82  -95 +93  fill=11    $center cluster
   53    0   -67 +66  -83 +82  -95 +93             $
             fill=11(+44.03 0.00 0.00)             $ right cluster
c
c - axial and radial geometry beyond the three clusters
   ...
   62   15 4.64399320e-2  -72 +71 -86 +85 -97 +94  $left  absorber plate
   63   15 4.64399320e-2  -74 +73 -86 +85 -97 +94  $right absorber plate
...
c   - these data are case 16:
   61   px  -93.08   $(s62-30.), left model boundary
   62   px  -63.08   $(s63-15*2.54),   left cluster, left boundary
   63   px  -24.98   $(s64-Sc),        left cluster, right boundary
   64   px  -19.05   $ -7.5*2.54,    center cluster, left  boundary
   65   px  +19.05   $  7.5*2.54,    center cluster, right boundary
   66   px  +24.98   $(s65+Sc),       right cluster, left  boundary
   67   px  +63.08   $(s66+15*2.54),  right cluster, right boundary
   68   px  +93.08   $(s67+30.), right model boundary
   71   px  -19.7800 $(s72-absorber plate thickness)
   72   px  -19.7509 $(s64-Sp)
   73   px  +19.7509 $(s65+Sp)
   74   px  +19.7800 $(s73+absorber plate thickness)
c
c - global y coordinates:
   ...
   85   py  -17.8     $ use -15.3 for thin copper plate (cases 12-15)!
   86   py  +17.8     $ use +15.3 for thin copper plate (cases 12-15)!
c
