PU-SOL-THERM-032.13, rev0, 9/30/2005, 9.95a% Pu240, 50.55 g Pu/l
c
c  Annular volume is air (cases 1-12) or water (cases 13-17).
c  Cases 3/13 = same solution, w/o & with annular water.
c  Cases 4/14 = same solution, w/o & with annular water.
c  Cases 5/15 = same solution, w/o & with annular water.
c  Cases 6/16 = same solution, w/o & with annular water.
c  Cases 7/17 = same solution, w/o & with annular water.
c
c  Benchmark keff is 1.0000(19) for all cases.
c
c  This deck is case 13 (detailed, i.e., slanted bottom, model).
c
  1     1  1.00393871e-1  -3 +2  -14 +13                  $Solution
c
  2     2  8.69059423e-2  (((-16 +15) : (-13 +12)) -4) :  $Tank upper and lower lid
                          (((-4 +3) : (-2 +1)) -15 +13)   $Tank sidewalls
c
 11     3  1.00082677e-1  (-5  -14 +11 (+4 : -12)):       $Radial  Water Reflector
                          (-1       -14 +13)              $Annular Water Reflector
c
 12     4  5.44376405e-5  (-1       -15 +14) :            $Central Air
...
c
  1     cz    9.6875             $Annulus IR 
  2     cz    9.9875             $Annulus OR/Solution IR
...
c 13     pz    0.0                $Start solution (flat tank bottom)
 14     pz   33.9               $  End solution (ICSBEP Hc - 0.1)
...
c
c
c    For m1 (1.00393871e-1): solution
c     - natN  may be converted to isoN  (1.7161e-3)
c     - natO  may be converted to isoO  (3.5881e-2)  (16+18O = 16O)
c     - natFe may be converted to isoFe (8.4648e-7)
c     - lwtr.20t is ENDF71SaB kernel
c     - lwtr.10t is from endf70sab
c     - hh2o.20t is Bob MacFarlane's new continuous energy kernel
 m1     94238.80c 6.3940e-8  94239.80c 1.1285e-4  94240.80c 1.2631e-5
        94241.80c 1.6164e-6  94242.80c 1.1319e-7                       $Pu
         1001.80c 6.2669e-2                                            $H
         7014.80c 1.7098e-3   7015.80c 6.3152e-6                       $isoN
         8016.80c 3.5867e-2   8017.80c 1.3635e-5                       $isoO
        26054.80c 4.9477e-8  26056.80c 7.7668e-7  26057.80c 1.7937e-8
        26058.80c 2.3871e-9                                            $isoFe
 mt1     lwtr.20t                                                      $S(a,b)
c
...
c *    energy interval for surface 3 (between surfaces -14 and +13) plus the   *
c *    axial surfaces (surface 13, inside surface 3, i.e., -3; and surface 14, *
c *    inside surface 3, i.e., -3).  Note that since surface 13 defines the    *
c *    bottom of the solution, need to compute J- - J+ to get leakage leaving  *
c *    the solution.  Also, since there is a central water annulus, need to    *
c *    consider neutrons crossing the inner radius, surface 2, between the     *
c *    upper and lower solution limits, surfaces 13 and 14.  This central      *
c *    annulus was ignored for cases 1 through 6 when it was filled with air   *
c *    since virtually every neutron entering this region simple passed        *
c *    through to the solution on the other side.                              *
c *                                                                            *
c ******************************************************************************
c
...
C11   0.0 1.0
c
F21:N 2             $ radial current (across solution inner annulus radius) ...
FS21 +14 +13        $ but -14,+13 will restrict edit to solution axial limits
E21   6.250e-7 150.
C21   0.0 1.0
c
c
