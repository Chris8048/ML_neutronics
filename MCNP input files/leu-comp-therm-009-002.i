...
c     - x-value is given by +/-(15*2.54 + Sc).
   51    0   -63 +62  -83 +82  -95 +93             $
             fill=11(-47.75 0.00 0.00)             $  left cluster
   52    0   -65 +64  -83 +82  -95 +93  fill=11    $center cluster
   53    0   -67 +66  -83 +82  -95 +93             $
             fill=11(+47.75 0.00 0.00)             $ right cluster
c
...
c   - these data are case 2:
   61   px  -96.80   $(s62-30.), left model boundary
   62   px  -66.80   $(s63-15*2.54),   left cluster, left boundary
   63   px  -28.70   $(s64-Sc),        left cluster, right boundary
   64   px  -19.05   $ -7.5*2.54,    center cluster, left  boundary
   65   px  +19.05   $  7.5*2.54,    center cluster, right boundary
   66   px  +28.70   $(s65+Sc),       right cluster, left  boundary
   67   px  +66.80   $(s66+15*2.54),  right cluster, right boundary
   68   px  +96.80   $(s67+30.), right model boundary
   71   px  -22.812  $(s72-absorber plate thickness)
   72   px  -22.327  $(s64-Sp)
   73   px  +22.327  $(s65+Sp)
   74   px  +22.812  $(s73+absorber plate thickness)
c
