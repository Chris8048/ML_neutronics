Leu-Comp-Therm-010.6, rev0, 8/31/1996.
c
...
c   (12x8 clusters for case 6):
   21    0                 -103 +102  -153 +152  -204 +203  
                           fill=11(-59.915 -10.16 +17.84)     $left   cluster
   22    0                 -105 +104  -153 +152  -204 +203
                           fill=11(-15.240 -10.16 +17.84)     $center cluster
   23    0                 -107 +106  -153 +152  -204 +203
                           fill=11(+59.915 -10.16 +17.84)     $right  cluster
...
c   Note, for case 6 the lattice is 12 x 8, not 13 x 8.
  101   px  -90.415  $left   model boundary (s102 - 30.5)
  102   px  -59.915  $left   cluster, left  boundary (s103 - 12*2.54)
  103   px  -29.435  $left   cluster, right boundary (s104 - 14.195)
  104   px  -15.24   $center cluster, left  boundary (-0.5*12*2.54)
  105   px   15.24   $center cluster, right boundary ( 0.5*12*2.54)
  106   px   29.435  $right  cluster, left  boundary (s105 + 14.195)
  107   px   59.915  $right  cluster, right boundary (s106 + 12*2.54)
  108   px   90.415  $right  model boundary (s107 + 30.5)
...
c   to one of 0.0, 0.660, 1.321, 1.956, 2.616, 3.912 or 5.405 cm separation.
c   Note that for zero separation, surfaces 162 and 163 are redundant with
c   surfaces 152 and 153, respectively.
  161   py  -19.766  $(s162 - [10.2(lead), 7.65(deplU) or 17.85(steel)]
  162   py  -12.116  $1.956 cm separation
  163   py   12.116  $
  164   py   19.766  $
c
